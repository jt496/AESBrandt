...
lemma exist_not_adj_of_adj_inter (h : G.CliqueFree (r + 2)) (hWc : ∀ {y}, y ∈ s₁ ∩ s₂ → G.Adj x y) :
...
lemma exists_isFiveWheelLike_insert_of_not_adj_le_two (h : G.CliqueFree (r + 2))
    (hWc : ∀ {y}, y ∈ s₁ ∩ s₂ → G.Adj x y)
    (hsmall : #((insert v (insert w₁ (insert w₂ (s₁ ∪ s₂)))).filter (fun z ↦ ¬ G.Adj x z)) ≤ 2) :
    ∃ a b, a ∉ s₂ ∧ b ∉ s₁ ∧
